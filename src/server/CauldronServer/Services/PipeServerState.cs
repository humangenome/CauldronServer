...
/// Cauldron's design is one plugin connection per CauldronServer instance.
/// </summary>
public sealed class PipeServerState
{
    ...
    // Native plugin reports auth state on every heartbeat (Cauldron.dll).
    ...
    public void UpsertLogPlayer(string cauldronUserId, string displayName, int pingMs = 0)
    {
        if (string.IsNullOrWhiteSpace(displayName)) return;
        var id = string.IsNullOrWhiteSpace(cauldronUserId) ? $"bw:{displayName}" : cauldronUserId;
    ...
    public void RemoveLogPlayer(string cauldronUserId)
    {
        if (string.IsNullOrWhiteSpace(cauldronUserId)) return;
        lock (_playersGate)
        {
            _logPlayers.Remove(cauldronUserId);
            _players = _players
                .Where(player => !string.Equals(player.CauldronUserId, cauldronUserId, StringComparison.OrdinalIgnoreCase))
                .ToList();
        }
    }
    ...
    {
        lock (_playersGate) _logPlayers[player.CauldronUserId] = player;
    }
        ...
        lock (_playersGate)
        {
            var mergedCount = _players
                ...
                .GroupBy(player => !string.IsNullOrWhiteSpace(player.DisplayName)
                    ...
                    : $"id:{player.CauldronUserId}", StringComparer.OrdinalIgnoreCase)
                .Count();
            if (mergedCount <= 1)
            {
                _logPlayers.Clear();
