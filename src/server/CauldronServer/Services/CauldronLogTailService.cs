...
/// <summary>
/// Tails the Witchspire host's UE log file (<c>{GameUserDir}/Saved/Logs/Witchspire.log</c>),
/// filters for operator-meaningful categories, and re-emits each line
/// through Serilog with a <c>[Witchspire]</c> source prefix. Joins, leaves, chat,
/// errors, save events, map travels — all show up interleaved with
/// CauldronServer's own log stream.
///
/// Also looks for the Witchspire build banner on the first pass and registers it
/// with <see cref="CauldronVersionInfo"/> so A2S responses can publish it.
/// </summary>
public sealed class CauldronLogTailService : BackgroundService
{
    ...
    // Witchspire log categories worth surfacing on the operator console.
    ...
    private readonly ILogger<CauldronLogTailService> _log;
    ...
    public CauldronLogTailService(ILogger<CauldronLogTailService> log, IOptions<CauldronServerOptions> opts, PipeServerState state)
    {
        _log = log;
        _opts = opts.Value;
        _state = state;
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        ...
        {
            _log.LogInformation("[Witchspire] log tail idle: GameUserDir not configured");
            return;
        }
        var logPath = Path.Combine(_opts.GameUserDir, "Saved", "Logs", "Witchspire.log");
        _log.LogInformation("[Witchspire] log tail watching {Path}", logPath);
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            {
                _log.LogWarning(ex, "[Witchspire] log tail loop error");
            }
    ...
    private async Task CatchUpExistingLogAsync(string logPath, long endPosition, CancellationToken ct)
    {
        ...
        {
            _log.LogWarning("[Witchspire] existing log is too large for startup catch-up; starting live tail at EOF");
            return;
        }
    ...
    private void EmitLine(string line, bool trackPlayerEvents)
    {
        ...
        if (m.Success)
        {
            ...
            CauldronVersionInfo.SetCauldronBuild(build);
            if (!string.Equals(_lastBuildLogged, build, StringComparison.Ordinal))
            {
                _log.LogInformation("[Witchspire] build detected: {Build}", build);
        ...
        if (TryExtractAcceptedAddress(line, out var joinAddress))
        {
            ...
            _log.LogInformation("[Witchspire] player connection accepted from {Addr}", addr);
            return;
        }
        ...
        if (joinSucceededMatch.Success)
        {
            ...
            _log.LogInformation("[Witchspire] player joined: {Name}", joinSucceededMatch.Groups["name"].Value);
            return;
        }
        ...
        if (trackPlayerEvents && gameplayLeaveMatch.Success)
        {
            ...
            _log.LogInformation("[Witchspire] player left: {Name}", name);
            return;
        }
        if (TryExtractLeaveAddress(line, out var leaveAddress))
        {
            ...
            _log.LogInformation("[Witchspire] connection closed for {Addr}", addr);
            return;
        }
        if (trackPlayerEvents && LogoutRegex.IsMatch(line))
        {
            _log.LogInformation("[Witchspire] player logout detected");
            return;
        }
        var travelMatch = TravelRegex.Match(line);
        if (travelMatch.Success)
        {
            _log.LogInformation("[Witchspire] travel -> {Url}", travelMatch.Groups["url"].Value);
            return;
        }

        // Default: pass through with [Witchspire] prefix and category as info.
        if (line.Contains("Error", StringComparison.OrdinalIgnoreCase))
            _log.LogError("[Witchspire] {Line}", line);
        else if (line.Contains("Warning", StringComparison.OrdinalIgnoreCase))
            _log.LogWarning("[Witchspire] {Line}", line);
        else
            _log.LogInformation("[Witchspire] {Line}", line);
    }
