...
public class CauldronLogTailServiceTests
{
    [Theory]
    ...
    {
        CauldronLogTailService.TryExtractAcceptedAddress(line, out var address).Should().BeTrue();
    ...
    {
        CauldronLogTailService.TryExtractLeaveAddress(line, out var address).Should().BeTrue();
        address.Should().Be(expected);
    }
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "cauldron-tail-" + Guid.NewGuid().ToString("N"));
        var logDir = Path.Combine(root, "Saved", "Logs");
        Directory.CreateDirectory(logDir);
        var logPath = Path.Combine(logDir, "Witchspire.log");
        ...
        var service = new CauldronLogTailService(
            NullLogger<CauldronLogTailService>.Instance,
            Options.Create(new CauldronServerOptions { GameUserDir = root }),
            state);
