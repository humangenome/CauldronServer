...
public class CauldronDbTests : IDisposable
{
    private readonly string _file;
    private readonly CauldronDb _db;

    public CauldronDbTests()
    {
        _file = Path.Combine(Path.GetTempPath(), $"cauldron-test-{Guid.NewGuid():N}.db");
        _db = new CauldronDb(_file);
    }
    ...
    public void Concurrent_ticket_consume_only_succeeds_once()
    {
        ...
        // UPDATE ... RETURNING in CauldronDb must let exactly one win.
