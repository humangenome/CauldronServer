...
namespace Cauldron.Protocol;

/// <summary>
/// Wire protocol between Cauldron.dll (UE4SS plugin loaded into the Witchspire
/// process) and CauldronServer.exe (.NET 8 supervisor). Bumped on any
...
public enum FrameType : byte
{
    ...
    // Mod-host plugin API (Phase 3+). Cauldron supports two mod surfaces:
    //   1. Server-side native (C++) mods loaded by Cauldron.dll inside Witchspire's
    //      process. They observe game events (player join, chat, world tick)
    //      and can call back into UE5 via UE4SS reflection.
    //   2. Server-side scripted (Lua) mods, also loaded by Cauldron.dll,
    //      sandboxed via sol2/LuaJIT. Limited surface: game events + a
    //      curated UE5 helper API.
    //   3. Client-side mods, loaded by CauldronLauncher into the customer's Witchspire
    //      via a Cauldron-installed UE4SS dropper. The server tells the client
...
public record HeartbeatMessage(
    ...
    // Server fails closed (stop Witchspire) when Configured=1 && HookReady=0.
...
public record PlayerJoinedMessage(
    [property: Key(0)] string CauldronUserId,
...
public record JoinTicketResponseMessage(
    [property: Key(0)] bool Accepted,
    [property: Key(1)] string? CauldronUserId,
...
{
    /// <summary>Server-side C++ native mod. .dll loaded by Cauldron.dll inside Witchspire.</summary>
    ServerNative = 1,
    /// <summary>Server-side Lua mod. .lua loaded into Cauldron's sol2 sandbox.</summary>
    ServerLua = 2,
    /// <summary>Client-side mod. .pak/.dll installed by CauldronLauncher into the customer's Witchspire install.</summary>
    Client = 3,
}
