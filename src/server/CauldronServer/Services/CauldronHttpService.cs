...
/// <see cref="CauldronServerOptions.HttpPort"/>, validates an HMAC-SHA256
...
/// and sends headers:
///   X-Cauldron-Timestamp: &lt;unix seconds&gt;
///   X-Cauldron-Signature: &lt;hex&gt;
/// Server rejects timestamps older than 5 minutes (replay window).
/// </summary>
public sealed class CauldronHttpService : BackgroundService
{
    private const int ReplayWindowSeconds = 300;
    private const int MaxConcurrentRequests = 8;

    private readonly ILogger<CauldronHttpService> _log;
    ...
    public CauldronHttpService(
        ILogger<CauldronHttpService> log,
    ...
    private async Task HandleRequestAsync(HttpListenerContext ctx, CancellationToken ct)
    {
        ...
        try
        {
            res.Headers["X-Cauldron-Instance"] = _identity.InstanceId;
            ...
            if (method == "GET" && path == "/api/v1/health")
            {
                ...
                // Server is "online" if the plugin is pinging OR the Witchspire process is
                ...
                await WriteJsonAsync(res, online ? 200 : 503, new
                {
                    ...
                    server_name = string.IsNullOrWhiteSpace(_opts.ServerName) ? $"Cauldron - {_identity.InstanceId}" : _opts.ServerName.Trim(),
                    cauldron_version = CauldronVersionInfo.CauldronVersion,
                    cauldron_build = CauldronVersionInfo.CauldronBuild,
            ...
            if (method == "GET" && path == "/api/v1/players")
            {
                ...
                await WriteJsonAsync(res, 200, new
                {
                    ...
                    {
                        name = string.IsNullOrEmpty(p.DisplayName) ? p.CauldronUserId : p.DisplayName,
            ...
            if (method == "GET" && path == "/api/v1/manifest")
            {
                ...
                var mods = _config.GetSection("Cauldron:Mods").Get<ModsOptions>() ?? new ModsOptions();
                await WriteJsonAsync(res, 200, new
                {
                    manifest_version = 1,
                    instance = _identity.InstanceId,
            ...
            //
            // CauldronRoster Lua mod writes roster.json (player list +
            ...
            // nothing extra.
            //
            // The in-game CauldronHud overlay runs inside the player's game
            ...
            //
            // The game has no native chat. Players type into the CauldronHud
            ...
            if (method == "GET" && path.StartsWith("/api/v1/snapshots/") && path.EndsWith("/download"))
            {
                ...
                res.Headers["X-Cauldron-Sha256"] = record.Sha256Hex;
            ...
            if (method == "POST" && path == "/api/v1/snapshots")
            {
                ...
                var record = new Cauldron.Persistence.SnapshotRecord(
    ...
    /// in a MemoryStream — uploads up to <see cref="CauldronServerOptions.MaxUploadBytes"/>
    ...
    {
        var tsHeader = req.Headers["X-Cauldron-Timestamp"];
        var sigHeader = req.Headers["X-Cauldron-Signature"];
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "cauldron-uploads");
    ...
    /// Succeeds when CauldronServer happens to be running with elevation;
    ...
    /// <summary>
    /// Reads roster.json (written by the CauldronRoster Lua mod every 5s,
    ...
    ///
    /// Why not a separate map-state.json: UE5 dedicated Witchspire with zero
            ...
            try
            {
                ...
                if (root.TryGetProperty("players", out var pa) && pa.ValueKind == JsonValueKind.Array)
                {
                    ...
                    foreach (var p in pa.EnumerateArray())
                    {
                        list.Add(new
                        {
                            id    = p.TryGetProperty("CauldronUserId", out var idEl) ? idEl.GetString() ?? "" : "",
