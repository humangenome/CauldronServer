namespace CauldronServer.Static;
...
internal static class MapSpa
{
    ...
    /// <summary>
    /// Hand-tuned to look reasonable at the Witchspire starting area's
...
<div id="wrap">
  ...
  <div id="list"></div>
  <div id="footer">Cauldron · <span id="cauldron-version">—</span></div>
</div>
<script>
(function () {
  ...
  var versionEl = document.getElementById('cauldron-version');
  ...
  function poll() {
    fetch('/api/v1/map/state', { cache: 'no-store' })
      ...
      .then(function (data) {
        state = data;
        if (data.cauldron_version) versionEl.textContent = data.cauldron_version;
