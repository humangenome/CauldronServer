...
/// <summary>
/// Hosted service that listens for Cauldron.dll plugin connections on a per-instance
/// Windows named pipe. Handles handshake, frame parsing, dispatch to handlers, and
/// connection lifecycle.
///
/// Single-connection: Cauldron expects exactly one game process per instance, so the
            ...
            case FrameType.Handshake:
                {
                    ...
                    // CauldronServerRuntime/main.lua at the very start of
                    // its Lua execution, BEFORE CauldronAuth runs. Doing
                    // it here on handshake races CauldronAuth - the
                    // handshake fires AFTER Cauldron.dll loads, which is
                    // AFTER UE4SS Lua mods run, which means CauldronAuth
                    ...
                    // CauldronAuth either overwrites it with current
            ...
            case FrameType.PlayerJoined:
                {
                    ...
                    _log.LogInformation("Player joined: {User} ({Name}) from {Addr}", pj.CauldronUserId, pj.DisplayName, pj.ClientAddress);
                    return connection;
                }
            ...
            case FrameType.PlayerLeft:
                {
                    ...
                    _log.LogInformation("Player left: {User} reason={Reason}", pl.CauldronUserId, pl.Reason);
                    return connection;
                }
