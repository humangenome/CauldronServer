...
namespace Cauldron.Persistence;

/// <summary>
/// SQLite WAL store for Cauldron's operational state. Owns its connection string;
...
///
/// Tables:
///   players        — known clients (cauldron_user_id, display, first/last seen, total seconds)
///   bans           — temporary or permanent bans keyed by cauldron_user_id
...
/// </summary>
public sealed class CauldronDb
{
    private readonly string _connectionString;

    public CauldronDb(string filePath)
    {
    ...
    private void EnsureSchema()
    {
        ...
        c.Execute("""
            CREATE TABLE IF NOT EXISTS players (
                cauldron_user_id  TEXT PRIMARY KEY,
        ...
        c.Execute("""
            CREATE TABLE IF NOT EXISTS join_tickets (
                ...
                cauldron_user_id  TEXT NOT NULL,
                display_name     TEXT NOT NULL,
    ...
    public void UpsertPlayerSeen(string cauldronUserId, string displayName, long unixSeconds)
    {
        using var c = Open();
        c.Execute("""
            INSERT INTO players (cauldron_user_id, display_name, first_seen_unix, last_seen_unix)
            VALUES (@id, @name, @ts, @ts)
            ON CONFLICT(cauldron_user_id) DO UPDATE SET
                display_name = excluded.display_name,
                last_seen_unix = excluded.last_seen_unix;
            """, new { id = cauldronUserId, name = displayName, ts = unixSeconds });
    }

    public PlayerRecord? GetPlayer(string cauldronUserId)
    {
        using var c = Open();
        return c.QueryFirstOrDefault<PlayerRecord>(
            "SELECT cauldron_user_id AS CauldronUserId, display_name AS DisplayName, " +
            "first_seen_unix AS FirstSeenUnix, last_seen_unix AS LastSeenUnix, " +
            "total_seconds AS TotalSeconds FROM players WHERE cauldron_user_id = @id",
            new { id = cauldronUserId });
    }
    ...
    public void AddBan(BanRecord ban)
    {
        using var c = Open();
        c.Execute("""
            INSERT INTO bans (cauldron_user_id, reason, expires_unix, added_by, added_unix)
            VALUES (@CauldronUserId, @Reason, @ExpiresUnix, @AddedBy, @AddedUnix)
            ON CONFLICT(cauldron_user_id) DO UPDATE SET
    ...
    public void RemoveBan(string cauldronUserId)
    {
        using var c = Open();
        c.Execute("DELETE FROM bans WHERE cauldron_user_id = @id", new { id = cauldronUserId });
    }

    public BanRecord? GetActiveBan(string cauldronUserId, long nowUnix)
    {
        using var c = Open();
        return c.QueryFirstOrDefault<BanRecord>("""
            SELECT cauldron_user_id AS CauldronUserId, reason AS Reason,
                   expires_unix AS ExpiresUnix, added_by AS AddedBy, added_unix AS AddedUnix
            FROM bans
            WHERE cauldron_user_id = @id
              AND (expires_unix IS NULL OR expires_unix > @now)
            """, new { id = cauldronUserId, now = nowUnix });
    }
    ...
    public void IssueTicket(JoinTicketRecord t)
    {
        using var c = Open();
        c.Execute("""
            INSERT INTO join_tickets (ticket_id, cauldron_user_id, display_name, issued_unix, expires_unix)
            VALUES (@TicketId, @CauldronUserId, @DisplayName, @IssuedUnix, @ExpiresUnix);
            """, t);
    }
    ...
    public JoinTicketRecord? ConsumeTicket(string ticketId, long nowUnix)
    {
        ...
        return c.QueryFirstOrDefault<JoinTicketRecord>("""
            ...
            RETURNING ticket_id AS TicketId, cauldron_user_id AS CauldronUserId,
...
public sealed record PlayerRecord(string CauldronUserId, string DisplayName, long FirstSeenUnix, long LastSeenUnix, long TotalSeconds);

public sealed record BanRecord(string CauldronUserId, string Reason, long? ExpiresUnix, string AddedBy, long AddedUnix);

public sealed record JoinTicketRecord(string TicketId, string CauldronUserId, string DisplayName, long IssuedUnix, long ExpiresUnix, long? ConsumedUnix);
