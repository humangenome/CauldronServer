...
/// <summary>
/// Coordinates between operations that need to pause Witchspire (snapshot restore,
/// world wipe, in-place upgrade) and the supervisor loop that keeps it
/// running. A restore acquires the gate, kills Witchspire if it's running, mutates
/// SaveGames, then releases the gate. The supervisor waits on the gate
/// before each relaunch so a new Witchspire doesn't spawn into a half-written
/// world.
///
/// Kill scoping: only kills Witchspire instances whose executable lives under the
/// configured <see cref="CauldronServerOptions.GameInstallRoot"/> (or, when that
/// is empty, instances whose command line references the configured
/// <see cref="CauldronServerOptions.GameUserDir"/>). A self-hoster running
/// vanilla Witchspire elsewhere on the same machine will not be touched.
/// </summary>
public sealed class CauldronRestartCoordinator
{
    private static readonly string[] CauldronProcessNames =
    [
        "Hercules-Win64-Shipping",
    ];

    private readonly ILogger<CauldronRestartCoordinator> _log;
    private readonly CauldronServerOptions _opts;
    private readonly SemaphoreSlim _restoreGate = new(1, 1);

    public CauldronRestartCoordinator(ILogger<CauldronRestartCoordinator> log, IOptions<CauldronServerOptions> opts)
    {
        _log = log;
        _opts = opts.Value;
    }
    ...
    /// <summary>
    /// Kill the Witchspire instances this CauldronServer owns. Scoping is strict:
    ...
    /// touched. Unrelated Witchspire processes on the machine — e.g. a customer's
    ...
    public void KillGame(TimeSpan waitForExit)
    {
        ...
        // CauldronServer doesn't own the game's lifecycle and GameInstallRoot is
        ...
        foreach (var name in CauldronProcessNames)
        {
            Process[] procs;
            ...
            foreach (var p in procs)
            {
                try
                {
                    ...
                    _log.LogInformation("Restoring: killing Witchspire pid={Pid} name={Name}", p.Id, name);
                    p.Kill(entireProcessTree: true);
                    p.WaitForExit((int)waitForExit.TotalMilliseconds);
                }
                catch (Exception ex)
                {
                    _log.LogWarning(ex, "Failed to kill Witchspire pid={Pid}", p.Id);
                }
                finally
                {
                    try { p.Dispose(); } catch { }
    ...
    private void TryKillFromPidFile(string pidFile, TimeSpan waitForExit)
    {
        ...
        // <root>\Witchspire\Binaries\Win64\Hercules-Win64-Shipping.exe).
        ...
        try
        {
            ...
            var isGame = false;
            foreach (var allowed in CauldronProcessNames)
            {
            ...
            {
                _log.LogWarning(
                    "TryKillFromPidFile: pid={Pid} now belongs to '{Name}' (not Witchspire); refusing to kill",
                    pid, name);
                return;
            }
            ...
            // which CauldronServer has because it runs as the same user
            ...
            {
                _log.LogWarning(
                    "TryKillFromPidFile: pid={Pid} ({Name}) exe={Exe} is NOT under expected instance root {Root}; refusing to kill (likely PID recycled to a different customer's Witchspire)",
                    pid, name, normExe, expectedRoot);
                return;
            }

            _log.LogInformation(
                "FAIL-CLOSED (pid-file): killing Witchspire pid={Pid} name={Name} exe={Exe} root={Root}",
                pid, name, normExe, expectedRoot);
            p.Kill(entireProcessTree: true);
            p.WaitForExit((int)waitForExit.TotalMilliseconds);
        }
    ...
    private static bool OwnsProcess(Process p, string? installRoot, string? userDir)
    {
        ...
        // privileges for processes owned by other users; in CauldronServer's
        ...
        if (installRoot is not null && exePath is not null)
        {
            ...
            // 'C:\Games\BW-vanilla\...' as living under 'C:\Games\BW'.
